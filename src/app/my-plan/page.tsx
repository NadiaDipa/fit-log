...
const MyPlan = () => {
    ...
    const { myPlan = [], setPlan, saved = [], setSaved } = useContext(WorkoutsContext) || {};
      ...
      <main className="min-h-screen bg-[#0C0D10] text-white px-6 md:px-16 py-10 font-sans">
        ...
        <div className="mb-8 container mx-auto">
          ...
          <p className="text-gray-400 text-sm md:text-base">
            {activeTab === 'plan' ? 'Cap of five lifts for today. Finish them, then load more.' : 'Your bookmarked workouts for later.'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 container mx-auto">
          <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-2xl p-6">
            ...
            <h3 className="text-4xl font-extrabold text-[#C2F800]">{currentList.length}</h3>
          </div>
          <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-2xl p-6">
          ...
          <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-2xl p-6">
            ...
            <h3 className="text-4xl font-extrabold text-white">{totalCalories}</h3>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 container mx-auto">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-400">
            <span>Sort By</span>
            <button className="bg-[#1A1A1A] border border-[#2A2A2A] px-4 py-2 rounded-xl text-white flex items-center gap-2 font-medium">
              Duration 
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-white font-bold uppercase tracking-wider">{item.name}</h3>
                        <p className="text-gray-400 text-xs mb-2">{item.equipment || "Bodyweight"}</p>
